...
import {LookAction, LookConfig, LookDirection, LookGestures, LookSize} from '../_shared/model/user/look';
...
export class LookService {
  ...
  get(look: string | LookConfig): string {
    if ((look as LookConfig).look) {
      ...
      return `${environment.app.imager}${look.look}&action=${look.action}&gesture=${look.gesture}&direction=${look.direction}&head_direction=${look.headDirection}&size=${look.size}&headonly=${look.headOnly ? 1 : 0}`;
    }
